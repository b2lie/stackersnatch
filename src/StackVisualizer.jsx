...
function StackVisualizer({ selectedSprite }) {
  ...
  useEffect(() => {
    if (hasStarted && currentState === 'q3' && isStackValid) {
      ...
      showAlert("⏲ timer paused. \nclick 'next level' to resume the game.");
    }
  ...
  const handleTransition = () => {
    ...
    if (level === 1 && lvl1Stacks[currentState]) {
      ...
      if (currentState == 'q1') {
        ...
        if (!poppedChar) {
          showAlert("🤚🏻 please pop something first !");
          deductPoints();
          return;
        }
    ...
    if (isValid) { // player is moved to the next state + lvl img updated
      ...
      showAlert("⛔ invalid stack for this state :T pls retry");
      deductPoints();
    }
  };
